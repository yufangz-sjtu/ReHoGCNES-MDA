...
class Minibatch:
    ...
    def __init__(self, adj_full_norm, adj_train, role, train_params, cpu_eval=False):
        ...
        self.node_train = np.array(role['tr']).ravel()
        self.node_val = np.array(role['va']).ravel()
        self.node_test = np.array(role['te']).ravel()
    ...
    def one_batch(self, mode='train'):
        ...
        """
        if mode in ['train','val','test','valtest']:
        ...
        '''
            ...
            self.batch_num += 1
            '''
        norm_loss = self.norm_loss_test if mode in ['train','val','test', 'valtest'] else self.norm_loss_train
